...
data_boothroyd <- read.csv(here("data", "data_boothroyd.csv")) |> 
  mutate(entropy_relTOmax = overall_entropy_noGroup/max_entropy) |> 
...
```{r}
# across stimulus type; by language status
data_boothroyd |> 
  ggplot() +
...
```{r}
# by stimulus type; by language status
data_boothroyd |> 
  ggplot() +
 geom_smooth(aes(y = intelligibility, 
                  x = lextale_raw,
...
```{r}
# by stimulus type; facet wrap by language status
data_boothroyd |> 
  ggplot() +
  geom_smooth(aes(y = intelligibility, 
                  x = lextale_raw, 
...
###### checking if spline is needed
```{r}
rq2_intell3_noSpline  <- glmmTMB::glmmTMB(intell_t ~ kbit2_raw_c + block_number +
                                    bilingual + lextale_raw_c + stimulus_type_hp +
                                    bilingual:stimulus_type_hp +
                                    bilingual:lextale_raw_c +
                                    lextale_raw_c:stimulus_type_hp +
...
rq2_intell3_Spline_2df  <- glmmTMB::glmmTMB(intell_t ~ kbit2_raw_c + block_number +
                                bilingual + splines::ns(lextale_raw_c, df = 2) + 
                                stimulus_type_hp +
                                bilingual:stimulus_type_hp +
                                bilingual:splines::ns(lextale_raw_c, df = 2) +
                                splines::ns(lextale_raw_c, df = 2):stimulus_type_hp +
                                bilingual:splines::ns(lextale_raw_c, 
                                                      df = 2):stimulus_type_hp +
                                (1 | listener_id) + 
                                (1 | speaker_id),
                      dispformula = ~ splines::ns(lextale_raw_c, df = 2) + 
                        stimulus_type_hp,
                      data = data_boothroyd,
                      family = glmmTMB::beta_family())

rq2_intell3_Spline_3df  <- glmmTMB::glmmTMB(intell_t ~ kbit2_raw_c + block_number +
                                    bilingual + splines::ns(lextale_raw_c, df = 3) + 
                                      stimulus_type_hp +
                                    bilingual:stimulus_type_hp +
                                    bilingual:splines::ns(lextale_raw_c, df = 3) +
                                    splines::ns(lextale_raw_c, df = 3):stimulus_type_hp +
                                    bilingual:splines::ns(lextale_raw_c, 
                                                          df = 3):stimulus_type_hp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
                      dispformula = ~ splines::ns(lextale_raw_c, df = 3) + 
                        stimulus_type_hp,
                      data = data_boothroyd,
                      family = glmmTMB::beta_family())

rq2_intell3_Spline_4df  <- glmmTMB::glmmTMB(intell_t ~ kbit2_raw_c + block_number +
                                    bilingual + splines::ns(lextale_raw_c, df = 4) + 
                                      stimulus_type_hp +
                                    bilingual:stimulus_type_hp +
                                    bilingual:splines::ns(lextale_raw_c, df = 4) +
                                    splines::ns(lextale_raw_c, df = 4):stimulus_type_hp +
                                    bilingual:splines::ns(lextale_raw_c, 
                                                          df = 4):stimulus_type_hp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
                      dispformula = ~ splines::ns(lextale_raw_c, df = 4) + 
                        stimulus_type_hp,
                      data = data_boothroyd,
                      family = glmmTMB::beta_family())

lmtest::lrtest(rq2_intell3_noSpline,
               rq2_intell3_Spline_2df)

lmtest::lrtest(rq2_intell3_noSpline,
               rq2_intell3_Spline_3df)

lmtest::lrtest(rq2_intell3_noSpline,
               rq2_intell3_Spline_4df)

lmtest::lrtest(rq2_intell3_Spline_2df,
               rq2_intell3_Spline_3df)

lmtest::lrtest(rq2_intell3_Spline_2df,
               rq2_intell3_Spline_4df)

AIC(rq2_intell3_noSpline, rq2_intell3_Spline_2df,
    rq2_intell3_Spline_3df, rq2_intell3_Spline_4df) |>
  arrange(AIC)

bbmle::AICtab(rq2_intell3_noSpline, rq2_intell3_Spline_2df,
    rq2_intell3_Spline_3df, rq2_intell3_Spline_4df)
```


###### setting up model with no spline
```{r}
...
# lextale_raw_c is testing if the effect of language proficiency is zero for whichever is the references used in the model.  (NH1 and NH2, 6)
# lextale_raw_c:stimulus_type_hp is testing if the effect of language proficiency is the same for HP, LP, ZP in the reference language status (NH3, 2)
# Bilingual/monolingual:lextale_raw_c is testing if the effect of language proficiency is same for bilingual and monolingual in the reference stimulus type (NH4, 3)
# monolingual:lextale_raw_c:stimulus_type_hp is testing if the effect of language proficiency is the same HP, LP, ZP AND if the effect of language proficiency is same for bilingual and monolingual (NH5, 1)
```
...
```{r}
lextale_c_values <- unique(data_boothroyd$lextale_raw_c) |> 
  sort()

rg_rq2_intell <- emmeans::ref_grid(rq2_intell_hp_b, at = list(lextale_raw_c = 
                                      lextale_c_values))
                            

rq2_intell_pred <- emmeans::emmeans(rg_rq2_intell, ~ stimulus_type_hp * lextale_raw_c |
                                      bilingual,
                                      type = "response") |> 
  as.data.frame() |> 
  mutate(bilingual = case_match(bilingual,
         ...
         lextale_raw = mean(data_boothroyd$lextale_raw) + lextale_raw_c) |> 
  select(-df) |> 
  filter(lextale_raw == 60 |
           lextale_raw == 80 |
           lextale_raw == 100 ) |> 
  filter(!(lextale_raw == 60 & bilingual == "Monolingual")) |> 
  arrange(bilingual) |> 
  arrange(stimulus_type_hp) |> 
  relocate(lextale_raw, .before = response) |> 
  select(-lextale_raw_c)
...
###### graph: marginal means reponses (between 60 to 100)
```{r}
lextale_c_values <- unique(data_boothroyd$lextale_raw_c) |> 
  sort()

#getting marginal means given lextale scores between 60 to 100 by 1; average nonverbal 
rg_rq2_intell <- emmeans::ref_grid(rq2_intell_hp_b, 
                                   at = list(lextale_raw_c = lextale_c_values))
#use the values of the centered values



rq2_intell_graph <- emmeans::emmeans(rg_rq2_intell, ~ stimulus_type_hp * lextale_raw_c * 
                                      bilingual,
                                      type = "response") |> 
  as.data.frame() |> 
  mutate(bilingual = case_match(bilingual,
  ...
  relocate(lextale_raw, .after = lextale_raw_c)


view(rq2_intell_graph)

rq2_intell_graph_b <- rq2_intell_graph |> 
  filter(bilingual == "Bilingual")

rq2_intell_graph_m <- rq2_intell_graph |> 
  filter(bilingual == "Monolingual") |> 
  filter(lextale_raw >= 70)

view(rq2_intell_graph_m)
```


```{r}
#bilingual plot
sig_text <- data.frame(lextale_raw_c = c(101, 101, 101),
                       response = c(.76, .51, .42),
...
p1 <- data_boothroyd |> 
  ...
  geom_jitter(
             ...
             width = 0.2,
             alpha = .5) +
  geom_smooth(data = rq2_intell_graph_b,
              aes(y = response, 
                  ymin = response - SE,
                  ymax = response + SE,
                  x = lextale_raw, 
  ...
  geom_text(data = sig_text, 
            ...
            label = c("*","*","*"),
            size = 4) +
  labs(x = "Proficiency on vocabolary knowledge (LexTALE scores)",
       y = "Word recognition scores",
       color = "Utterance type") +
  scale_y_continuous(breaks = seq(0, 1, by = .2), oob = scales::squish, 
                     limits = c(0,1), labels = scales::percent) +
...
p2 <- data_boothroyd |> 
  ...
  geom_jitter(
             ...
             width = .2,
             alpha = .5) +
  geom_smooth(data = rq2_intell_graph_m,
              aes(y = response, 
                  ymin = response - SE,
                  ymax = response + SE,
                  x = lextale_raw, 
...
a3rq2_wr_fig <- ggpubr::ggarrange(p1, p2,
                  nrow = 1,
                  ncol = 2, 
                  widths = c(1.25,1),
                  common.legend = TRUE) 

ggpubr::annotate_figure(a3rq2_wr_fig,
                        bottom = ggpubr::text_grob(
                          "Proficiency on vocabulary knowledge (LexTALE scores)", 
                        family = "Times",
                        vjust = -.1)) 

ggsave(here::here("figures_study3", "rq2_wr_scatterplot.jpeg"),
       dpi = 300,
       width = 7.5,
       height = 4,
       unit = "in")
...
###### setting up model with spline
```{r}
#HP utterances as reference
#bilingual = bilingual 1; monolingual 0; other coef pertain to monolinguals
rq2_intell_hp_b_spline <- glmmTMB::glmmTMB(intell_t ~ kbit2_raw_c + block_number +
                                    bilingual + splines::ns(lextale_raw_c, df = 2) + 
                                      stimulus_type_hp +
                                    bilingual:stimulus_type_hp +
                                    bilingual:splines::ns(lextale_raw_c, df = 2) +
...
summary(rq2_intell_hp_b_spline)
...
#LP utterances as reference
#bilingual = bilingual 1; monolingual 0; other coef pertain to monolinguals
rq2_intell_lp_b_spline <- glmmTMB::glmmTMB(intell_t ~ kbit2_raw_c + block_number +
                                    bilingual + splines::ns(lextale_raw_c, df = 2) + 
                                      stimulus_type_lp +
                                    bilingual:stimulus_type_lp +
                                    bilingual:splines::ns(lextale_raw_c, df = 2) +
                                    splines::ns(lextale_raw_c, df = 2):stimulus_type_lp +
                                    bilingual:splines::ns(lextale_raw_c, 
                                                          df = 2):stimulus_type_lp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
                      dispformula = ~ splines::ns(lextale_raw_c, df = 2) + 
                        stimulus_type_lp,
...
summary(rq2_intell_lp_b_spline)
```

```{r}
#ZP utterances as reference
#bilingual = bilingual 1; monolingual 0; other coef pertain to monolinguals
rq2_intell_zp_b_spline <- glmmTMB::glmmTMB(intell_t ~ kbit2_raw_c + block_number +
                                    bilingual + splines::ns(lextale_raw_c, df = 2) + 
                                      stimulus_type_zp +
                                    bilingual:stimulus_type_zp +
                                    bilingual:splines::ns(lextale_raw_c, df = 2) +
                                    splines::ns(lextale_raw_c, df = 2):stimulus_type_zp +
                                    bilingual:splines::ns(lextale_raw_c, 
                                                          df = 2):stimulus_type_zp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
                      dispformula = ~ splines::ns(lextale_raw_c, df = 2) + 
                        stimulus_type_zp,
...
summary(rq2_intell_zp_b_spline)
...
```{r}
#monolingual = bilingual 0; monolingual 1; other coef pertain to bilinguals
rq2_intell_hp_m_spline <- glmmTMB::glmmTMB(intell_t ~ kbit2_raw_c + block_number +
                                    monolingual + splines::ns(lextale_raw_c, df = 2) + 
                                      stimulus_type_hp +
                                    monolingual:splines::ns(lextale_raw_c, df = 2) +
                                    monolingual:stimulus_type_hp +
                                    splines::ns(lextale_raw_c, df = 2):stimulus_type_hp +
                                    monolingual:splines::ns(lextale_raw_c, 
                                                            df = 2):stimulus_type_hp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
                      dispformula = ~ splines::ns(lextale_raw_c, df = 2) + 
...
summary(rq2_intell_hp_m_spline)
...
#LP utterances as reference
#bilingual = bilingual 0; monolingual 1; other coef pertain to bilinguals
rq2_intell_lp_m_spline <- glmmTMB::glmmTMB(intell_t ~ kbit2_raw_c + block_number +
                                    monolingual + splines::ns(lextale_raw_c, df = 2) + 
                                      stimulus_type_lp +
                                    monolingual:splines::ns(lextale_raw_c, df = 2) +
                                    monolingual:stimulus_type_lp +
...
summary(rq2_intell_lp_m_spline)
```

```{r}
#ZP utterances as reference
#bilingual = bilingual 0; monolingual 1; other coef pertain to bilinguals
rq2_intell_zp_m_spline <- glmmTMB::glmmTMB(intell_t ~ kbit2_raw_c + block_number +
                                    monolingual + splines::ns(lextale_raw_c, df = 2) + 
                                      stimulus_type_zp +
                                    monolingual:splines::ns(lextale_raw_c, df = 2) +
                                    monolingual:stimulus_type_zp +
...
summary(rq2_intell_zp_m_spline)
```
...
```{r}
#with spline
rq2_intell_h_spline <- rbind(
car::Anova(rq2_intell_hp_b_spline, type = "III") |> 
data.frame() |>  
    rename(p.orig = Pr..Chisq.) |> 
    mutate(variable = rownames(car::Anova(rq2_intell_hp_b_spline, type = "III")),
           lang_status = "Monolingual",
           stimulus_type = "HP") |> 
    relocate(variable, .before = Chisq) |> 
    relocate(lang_status, .before = Chisq) |> 
    relocate(stimulus_type, .before = Chisq) |> 
    filter(variable == "splines::ns(lextale_raw_c, df = 2)" |
           variable == "bilingual:splines::ns(lextale_raw_c, df = 2)" | 
           variable == "splines::ns(lextale_raw_c, df = 2):stimulus_type_hp" |
           variable == "bilingual:splines::ns(lextale_raw_c, df = 2):stimulus_type_hp" ) |> 
    mutate(lang_status = ifelse(variable == "bilingual:splines::ns(lextale_raw_c, df = 2)" |
                                variable == "bilingual:splines::ns(lextale_raw_c, df = 2):stimulus_type_hp", 
                                NA, lang_status),
           stimulus_type = ifelse(variable == "splines::ns(lextale_raw_c, df = 2):stimulus_type_hp"|
                                variable == "bilingual:splines::ns(lextale_raw_c, df = 2):stimulus_type_hp", 
                                  NA, stimulus_type)),

car::Anova(rq2_intell_lp_b_spline, type = "III") |> 
data.frame() |>  
    rename(p.orig = Pr..Chisq.) |> 
    mutate(variable = rownames(car::Anova(rq2_intell_hp_b_spline, type = "III")),
           lang_status = "Monolingual",
...
car::Anova(rq2_intell_zp_b_spline, type = "III") |> 
data.frame() |>  
    rename(p.orig = Pr..Chisq.) |> 
    mutate(variable = rownames(car::Anova(rq2_intell_hp_b_spline, type = "III")),
           lang_status = "Monolingual",
...
car::Anova(rq2_intell_hp_m_spline, type = "III") |> 
data.frame() |>  
    rename(p.orig = Pr..Chisq.) |> 
    mutate(variable = rownames(car::Anova(rq2_intell_hp_m_spline, type = "III")),
           lang_status = "Bilingual",
           stimulus_type = "HP") |> 
    relocate(variable, .before = Chisq) |> 
    relocate(lang_status, .before = Chisq) |> 
    ...
    filter(variable == "splines::ns(lextale_raw_c, df = 2)" |
           variable == "splines::ns(lextale_raw_c, df = 2):stimulus_type_hp") |> 
    mutate(stimulus_type = ifelse(variable == "splines::ns(lextale_raw_c, df = 2):stimulus_type_hp", 
                                  NA, stimulus_type)),

car::Anova(rq2_intell_lp_m_spline, type = "III") |> 
data.frame() |>  
    rename(p.orig = Pr..Chisq.) |> 
    mutate(variable = rownames(car::Anova(rq2_intell_hp_m_spline, type = "III")),
           lang_status = "Bilingual",
...
car::Anova(rq2_intell_zp_m_spline, type = "III") |> 
data.frame() |>  
    rename(p.orig = Pr..Chisq.) |> 
    mutate(variable = rownames(car::Anova(rq2_intell_hp_m_spline, type = "III")),
           lang_status = "Bilingual",
...
mutate(variable = factor(variable, levels = 
                               c("splines::ns(lextale_raw_c, df = 2)",
                                 "splines::ns(lextale_raw_c, df = 2):stimulus_type_hp",
                                 "bilingual:splines::ns(lextale_raw_c, df = 2)",
                                 "bilingual:splines::ns(lextale_raw_c, df = 2):stimulus_type_hp")),
       lang_status = factor(lang_status, levels = c("Bilingual", 
                                                    "Monolingual")),
       stimulus_type = factor(stimulus_type, levels = c("HP", "LP", "ZP")),
       outcome = "intelligibility") |> 
...
rg_rq2_intell_spline <- emmeans::ref_grid(rq2_intell_hp_b_spline, at = list(lextale_raw_c = 
                                      lextale_c_values))
                            

rq2_intell_pred_spline <- emmeans::emmeans(rg_rq2_intell_spline, ~ stimulus_type_hp * lextale_raw_c |
                                      bilingual,
                                      type = "response") |> 
  as.data.frame() |> 
  mutate(bilingual = case_match(bilingual,
...
view(rq2_intell_pred_spline)

rq2_intell_pred_spline |> 
  kableExtra::kbl() |> 
  kableExtra::kable_classic(html_font = "Times New Roman", full_width = F)
```
...
#getting marginal means given lextale scores between 60 to 100 by 1; average nonverbal 
rg_rq2_intell_spline <- emmeans::ref_grid(rq2_intell_hp_b_spline, 
                                   at = list(lextale_raw_c = lextale_c_values))
#use the values of the centered values



rq2_intell_graph_spline <- emmeans::emmeans(rg_rq2_intell_spline, ~ stimulus_type_hp * lextale_raw_c * 
                                      bilingual,
                                      type = "response") |> 
  as.data.frame() |> 
  mutate(bilingual = case_match(bilingual,
...
view(rq2_intell_graph_spline)

rq2_intell_graph_b_spline <- rq2_intell_graph_spline |> 
  filter(bilingual == "Bilingual")

rq2_intell_graph_m_spline <- rq2_intell_graph_spline |> 
  filter(bilingual == "Monolingual") |> 
  filter(lextale_raw >= 70)
...
p1 <- data_boothroyd |> 
  ...
  geom_point(
             aes(y = intelligibility, 
                 ...
                 colour = stimulus_type),
             alpha = .5) +
  geom_smooth(data = rq2_intell_graph_b_spline,
              aes(y = response, 
                  ymin = response - SE,
                  ymax = response + SE,
                  x = lextale_raw, 
...
p2 <- data_boothroyd |> 
  ...
  geom_point(
             aes(y = intelligibility, 
                 ...
                 colour = stimulus_type),
             alpha = .5) +
  geom_smooth(data = rq2_intell_graph_m_spline,
              aes(y = response, 
                  ymin = response - SE,
                  ymax = response + SE,
                  x = lextale_raw, 
...
ggpubr::annotate_figure(a3rq2_wr_fig,
                        bottom = ggpubr::text_grob(
                          "Proficiency on vocabolary knowledge (LexTALE scores)", 
                        family = "Times",
                        vjust = -.1)) 
...
###### checking if spline is needed
```{r}
rq2_effort3_noSpline  <- glmmTMB::glmmTMB(effort_t ~ kbit2_raw_c + block_number +
                                    bilingual + lextale_raw_c + stimulus_type_hp +
                                    bilingual:stimulus_type_hp +
                                    bilingual:lextale_raw_c +
                                    lextale_raw_c:stimulus_type_hp +
...
rq2_effort3_Spline_2df  <- glmmTMB::glmmTMB(effort_t ~ kbit2_raw_c + block_number +
                                bilingual + splines::ns(lextale_raw_c, df = 2) + 
                                stimulus_type_hp +
                                bilingual:stimulus_type_hp +
                                bilingual:splines::ns(lextale_raw_c, df = 2) +
...
rq2_effort3_Spline_3df  <- glmmTMB::glmmTMB(effort_t ~ kbit2_raw_c + block_number +
                                    bilingual + splines::ns(lextale_raw_c, df = 3) + 
                                      stimulus_type_hp +
                                    bilingual:stimulus_type_hp +
                                    bilingual:splines::ns(lextale_raw_c, df = 3) +
...
rq2_effort3_Spline_4df  <- glmmTMB::glmmTMB(effort_t ~ kbit2_raw_c + block_number +
                                    bilingual + splines::ns(lextale_raw_c, df = 4) + 
                                      stimulus_type_hp +
                                    bilingual:stimulus_type_hp +
                                    bilingual:splines::ns(lextale_raw_c, df = 4) +
                                    ...
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
                      dispformula = ~ splines::ns(lextale_raw_c, df = 4) + stimulus_type_hp,
                      data = data_boothroyd,
                      family = glmmTMB::beta_family())

lmtest::lrtest(rq2_effort3_noSpline,
               rq2_effort3_Spline_2df)

lmtest::lrtest(rq2_effort3_noSpline,
               rq2_effort3_Spline_3df)

lmtest::lrtest(rq2_effort3_noSpline,
               rq2_effort3_Spline_4df)

lmtest::lrtest(rq2_effort3_Spline_2df,
               rq2_effort3_Spline_3df)

lmtest::lrtest(rq2_effort3_Spline_2df,
               rq2_effort3_Spline_4df)

AIC(rq2_effort3_noSpline, rq2_effort3_Spline_2df,
    rq2_effort3_Spline_3df, rq2_effort3_Spline_4df) |>
  arrange(AIC)

bbmle::AICtab(rq2_effort3_noSpline, rq2_effort3_Spline_2df,
    rq2_effort3_Spline_3df, rq2_effort3_Spline_4df)
```
...
#LP utterances as reference
#bilingual = bilingual 1; monolingual 0; other coef pertain to monolinguals
rq2_effort_lp_b <- glmmTMB::glmmTMB(effort_t ~ kbit2_raw_c + block_number +
                                    bilingual + lextale_raw_c + stimulus_type_lp +
                                    bilingual:stimulus_type_lp +
                                    bilingual:lextale_raw_c +
                                    lextale_raw_c:stimulus_type_lp +
...
summary(rq2_effort_lp_b)
```

```{r}
#ZP utterances as reference
#bilingual = bilingual 1; monolingual 0; other coef pertain to monolinguals
rq2_effort_zp_b <- glmmTMB::glmmTMB(effort_t ~ kbit2_raw_c + block_number +
                                    bilingual + lextale_raw_c + stimulus_type_zp +
                                    bilingual:stimulus_type_zp +
                                    bilingual:lextale_raw_c +
                                    lextale_raw_c:stimulus_type_zp +
...
```{r}
#monolingual = bilingual 0; monolingual 1; other coef pertain to bilinguals
rq2_effort_hp_m <- glmmTMB::glmmTMB(effort_t ~ kbit2_raw_c + block_number +
                                    monolingual + lextale_raw_c + stimulus_type_hp +
                                    monolingual:lextale_raw_c +
                                    monolingual:stimulus_type_hp +
                                    lextale_raw_c:stimulus_type_hp +
...
#LP utterances as reference
#bilingual = bilingual 0; monolingual 1; other coef pertain to bilinguals
rq2_effort_lp_m <- glmmTMB::glmmTMB(effort_t ~ kbit2_raw_c + block_number +
                                    monolingual + lextale_raw_c + stimulus_type_lp +
                                    monolingual:lextale_raw_c +
                                    monolingual:stimulus_type_lp +
                                    lextale_raw_c:stimulus_type_lp +
...
summary(rq2_effort_lp_m)
```

```{r}
#ZP utterances as reference
#bilingual = bilingual 0; monolingual 1; other coef pertain to bilinguals
rq2_effort_zp_m <- glmmTMB::glmmTMB(effort_t ~ kbit2_raw_c + block_number +
                                    monolingual + lextale_raw_c + stimulus_type_zp +
                                    monolingual:lextale_raw_c +
                                    monolingual:stimulus_type_zp +
                                    lextale_raw_c:stimulus_type_zp +
...
summary(rq2_effort_zp_m)
```

###### hypothesis testing 
```{r}
pairs(emmeans::emmeans(rq2_effort_hp_b, ~ stimulus_type_hp | lextale_raw_c + bilingual))
```

```{r}
car::Anova(rq2_effort_hp_b, type = "III")
car::Anova(rq2_effort_lp_b, type = "III")
car::Anova(rq2_effort_zp_b, type = "III")
car::Anova(rq2_effort_hp_m, type = "III")
car::Anova(rq2_effort_lp_m, type = "III")
car::Anova(rq2_effort_zp_m, type = "III")
...
data.frame() |>  
    rename(p.orig = Pr..Chisq.) |> 
    mutate(variable = rownames(car::Anova(rq2_effort_hp_b, type = "III")),
           lang_status = "Monolingual",
           stimulus_type = "HP") |> 
    relocate(variable, .before = Chisq) |> 
    relocate(lang_status, .before = Chisq) |> 
...
data.frame() |>  
    rename(p.orig = Pr..Chisq.) |> 
    mutate(variable = rownames(car::Anova(rq2_effort_hp_m, type = "III")),
           lang_status = "Bilingual",
           stimulus_type = "HP") |> 
    relocate(variable, .before = Chisq) |> 
    relocate(lang_status, .before = Chisq) |> 
...
rg_rq2_effort <- emmeans::ref_grid(rq2_effort_hp_b, at = list(lextale_raw_c = 
                                      lextale_c_values))
                            

rq2_effort_pred <- emmeans::emmeans(rg_rq2_effort, ~ stimulus_type_hp * lextale_raw_c *
                                      bilingual,
                                      type = "response") |> 
  as.data.frame() |> 
  mutate(bilingual = case_match(bilingual,
                                0 ~ "Monolingual",
                                1 ~ "Bilingual"),
         response = round((response - .0125) / (.975/7),2),
         SE = round((SE- .0125) / (.975/7),2),
         asymp.LCL = round((asymp.LCL- .0125) / (.975/7),2),
         asymp.UCL = round((asymp.UCL- .0125) / (.975/7),2),
         lextale_raw = mean(data_boothroyd$lextale_raw) + lextale_raw_c) |> 
  select(-df) |> 
  filter(lextale_raw == 60 |
           lextale_raw == 80 |
...
#getting marginal means given lextale scores between 60 to 100 by 1; average nonverbal 
rg_rq2_effort <- emmeans::ref_grid(rq2_effort_hp_b, 
                                   at = list(lextale_raw_c = lextale_c_values))
#use the values of the centered values



rq2_effort_graph <- emmeans::emmeans(rg_rq2_effort, ~ stimulus_type_hp * lextale_raw_c * 
                                      bilingual,
                                      type = "response") |> 
  as.data.frame() |> 
  mutate(bilingual = case_match(bilingual,
                                0 ~ "Monolingual",
                                1 ~ "Bilingual"),
         response = (response - .0125) / (.975/7),
         SE = (SE- .0125) / (.975/7),
         asymp.LCL = (asymp.LCL- .0125) / (.975/7),
         asymp.UCL = (asymp.UCL- .0125) / (.975/7),
         lextale_raw = mean(data_boothroyd$lextale_raw) + lextale_raw_c
         ) |> 
  relocate(lextale_raw, .after = lextale_raw_c)


view(rq2_effort_graph)

rq2_effort_graph_b <- rq2_effort_graph |> 
  filter(bilingual == "Bilingual")

rq2_effort_graph_m <- rq2_effort_graph |> 
  filter(bilingual == "Monolingual") |> 
  filter(lextale_raw >= 70)

view(rq2_effort_graph_m)
```
...
p1 <- data_boothroyd |> 
  ...
  geom_jitter(
             ...
             width = .25,
             alpha = .5) +
  geom_smooth(data = rq2_effort_graph_b,
              aes(y = response, 
                  ymin = response - SE,
                  ymax = response + SE,
                  x = lextale_raw, 
...
p2 <- data_boothroyd |> 
  ...
  geom_jitter(
             ...
             width = .25,
             alpha = .5) +
  geom_smooth(data = rq2_effort_graph_m,
              aes(y = response, 
                  ymin = response - SE,
                  ymax = response + SE,
                  x = lextale_raw, 
  ...
  scale_y_continuous(breaks = seq(0, 6, by = 1), oob = scales::squish, 
                     limits = c(0,6)) +
  scale_x_continuous(breaks = seq(70, 100, by = 10), 
                     limits = c(70,100), oob = scales::squish) +
  theme_bw() +
  theme(axis.line = element_blank(),
        panel.grid.minor = element_blank(),
        panel.border = element_rect(colour = "black"),
...
ggsave(here::here("figures_study3", "rq2_le_scatterplot.jpeg"),
       dpi = 300,
       width = 7.5,
       height = 4,
       unit = "in")
...
###### checking if spline is needed
```{r}
rq2_fatigue_post3_noSpline  <- glmmTMB::glmmTMB(fatigue_post_t ~ kbit2_raw_c + block_number +
                                    bilingual + lextale_raw_c + stimulus_type_hp +
                                    bilingual:stimulus_type_hp +
                                    bilingual:lextale_raw_c +
                                    lextale_raw_c:stimulus_type_hp +
...
rq2_fatigue_post3_Spline_2df  <- glmmTMB::glmmTMB(fatigue_post_t ~ kbit2_raw_c + block_number +
                                bilingual + splines::ns(lextale_raw_c, df = 2) + 
                                stimulus_type_hp +
                                bilingual:stimulus_type_hp +
                                bilingual:splines::ns(lextale_raw_c, df = 2) +
...
rq2_fatigue_post3_Spline_3df  <- glmmTMB::glmmTMB(fatigue_post_t ~ kbit2_raw_c + block_number +
                                    bilingual + splines::ns(lextale_raw_c, df = 3) + 
                                      stimulus_type_hp +
                                    bilingual:stimulus_type_hp +
                                    bilingual:splines::ns(lextale_raw_c, df = 3) +
...
rq2_fatigue_post3_Spline_4df  <- glmmTMB::glmmTMB(fatigue_post_t ~ kbit2_raw_c + block_number +
                                    bilingual + splines::ns(lextale_raw_c, df = 4) + 
                                      stimulus_type_hp +
                                    bilingual:stimulus_type_hp +
                                    bilingual:splines::ns(lextale_raw_c, df = 4) +
...
lmtest::lrtest(rq2_fatigue_post3_noSpline,
               rq2_fatigue_post3_Spline_2df)

lmtest::lrtest(rq2_fatigue_post3_noSpline,
               rq2_fatigue_post3_Spline_3df)

lmtest::lrtest(rq2_fatigue_post3_noSpline,
               rq2_fatigue_post3_Spline_4df)

lmtest::lrtest(rq2_fatigue_post3_Spline_2df,
               rq2_fatigue_post3_Spline_3df)

lmtest::lrtest(rq2_fatigue_post3_Spline_2df,
               rq2_fatigue_post3_Spline_4df)

AIC(rq2_fatigue_post3_noSpline, rq2_fatigue_post3_Spline_2df,
    rq2_fatigue_post3_Spline_3df, rq2_fatigue_post3_Spline_4df) |>
  arrange(AIC)

bbmle::AICtab(rq2_fatigue_post3_noSpline, rq2_fatigue_post3_Spline_2df,
    rq2_fatigue_post3_Spline_3df, rq2_fatigue_post3_Spline_4df)
```
...
#LP utterances as reference
#bilingual = bilingual 1; monolingual 0; other coef pertain to monolinguals
rq2_fatigue_post_lp_b <- glmmTMB::glmmTMB(fatigue_post_t ~ kbit2_raw_c + block_number +
                                    bilingual + lextale_raw_c + stimulus_type_lp +
                                    bilingual:stimulus_type_lp +
                                    bilingual:lextale_raw_c +
                                    lextale_raw_c:stimulus_type_lp +
...
summary(rq2_fatigue_post_lp_b)
```

```{r}
#ZP utterances as reference
#bilingual = bilingual 1; monolingual 0; other coef pertain to monolinguals
rq2_fatigue_post_zp_b <- glmmTMB::glmmTMB(fatigue_post_t ~ kbit2_raw_c + block_number +
                                    bilingual + lextale_raw_c + stimulus_type_zp +
                                    bilingual:stimulus_type_zp +
                                    bilingual:lextale_raw_c +
                                    lextale_raw_c:stimulus_type_zp +
...
```{r}
#monolingual = bilingual 0; monolingual 1; other coef pertain to bilinguals
rq2_fatigue_post_hp_m <- glmmTMB::glmmTMB(fatigue_post_t ~ kbit2_raw_c + block_number +
                                    monolingual + lextale_raw_c + stimulus_type_hp +
                                    monolingual:lextale_raw_c +
                                    monolingual:stimulus_type_hp +
                                    lextale_raw_c:stimulus_type_hp +
...
#LP utterances as reference
#bilingual = bilingual 0; monolingual 1; other coef pertain to bilinguals
rq2_fatigue_post_lp_m <- glmmTMB::glmmTMB(fatigue_post_t ~ kbit2_raw_c + block_number +
                                    monolingual + lextale_raw_c + stimulus_type_lp +
                                    monolingual:lextale_raw_c +
                                    monolingual:stimulus_type_lp +
                                    lextale_raw_c:stimulus_type_lp +
...
summary(rq2_fatigue_post_lp_m)
```

```{r}
#ZP utterances as reference
#bilingual = bilingual 0; monolingual 1; other coef pertain to bilinguals
rq2_fatigue_post_zp_m <- glmmTMB::glmmTMB(fatigue_post_t ~ kbit2_raw_c + block_number +
                                    monolingual + lextale_raw_c + stimulus_type_zp +
                                    monolingual:lextale_raw_c +
                                    monolingual:stimulus_type_zp +
                                    lextale_raw_c:stimulus_type_zp +
...
summary(rq2_fatigue_post_zp_m)
```

###### hypothesis testing 
```{r}
pairs(emmeans::emmeans(rq2_fatigue_post_hp_b, ~ stimulus_type_hp | lextale_raw_c + bilingual))
```

```{r}
car::Anova(rq2_fatigue_post_hp_b, type = "III")
car::Anova(rq2_fatigue_post_lp_b, type = "III")
car::Anova(rq2_fatigue_post_zp_b, type = "III")
car::Anova(rq2_fatigue_post_hp_m, type = "III")
car::Anova(rq2_fatigue_post_lp_m, type = "III")
car::Anova(rq2_fatigue_post_zp_m, type = "III")
...
```{r}
rq2_fatigue_post_h <- rbind(
car::Anova(rq2_fatigue_post_hp_b, type = "III") |> 
data.frame() |>  
    rename(p.orig = Pr..Chisq.) |> 
    mutate(variable = rownames(car::Anova(rq2_fatigue_post_hp_b, type = "III")),
           lang_status = "Monolingual",
           stimulus_type = "HP") |> 
    relocate(variable, .before = Chisq) |> 
    relocate(lang_status, .before = Chisq) |> 
...
data.frame() |>  
    rename(p.orig = Pr..Chisq.) |> 
    mutate(variable = rownames(car::Anova(rq2_fatigue_post_hp_m, type = "III")),
           lang_status = "Bilingual",
           stimulus_type = "HP") |> 
    relocate(variable, .before = Chisq) |> 
    relocate(lang_status, .before = Chisq) |> 
...
rg_rq2_fatigue_post <- emmeans::ref_grid(rq2_fatigue_post_hp_b, at = list(lextale_raw_c = 
                                      lextale_c_values))
                            

rq2_fatigue_post_pred <- emmeans::emmeans(rg_rq2_fatigue_post, ~ stimulus_type_hp * lextale_raw_c *
                                      bilingual,
                                      type = "response") |> 
  as.data.frame() |> 
  mutate(bilingual = case_match(bilingual,
...
view(rq2_fatigue_post_pred)

rq2_fatigue_post_pred |> 
  kableExtra::kbl() |> 
  kableExtra::kable_classic(html_font = "Times New Roman", full_width = F)
```
...
#getting marginal means given lextale scores between 60 to 100 by 1; average nonverbal 
rg_rq2_fatigue_post <- emmeans::ref_grid(rq2_fatigue_post_hp_b, 
                                   at = list(lextale_raw_c = lextale_c_values))
#use the values of the centered values



rq2_fatigue_post_graph <- emmeans::emmeans(rg_rq2_fatigue_post, ~ stimulus_type_hp * lextale_raw_c * 
                                      bilingual,
                                      type = "response") |> 
  as.data.frame() |> 
  mutate(bilingual = case_match(bilingual,
...
view(rq2_fatigue_post_graph)

rq2_fatigue_post_graph_b <- rq2_fatigue_post_graph |> 
  filter(bilingual == "Bilingual")

rq2_fatigue_post_graph_m <- rq2_fatigue_post_graph |> 
  filter(bilingual == "Monolingual") |> 
  filter(lextale_raw >= 70)

view(rq2_fatigue_post_graph_m)
```
...
p1 <- data_boothroyd |> 
  ...
  geom_jitter(
             ...
             width = .25,
             alpha = .5) +
  geom_smooth(data = rq2_fatigue_post_graph_b,
              aes(y = response, 
                  ymin = response - SE,
                  ymax = response + SE,
                  x = lextale_raw, 
...
p2 <- data_boothroyd |> 
  ...
  geom_jitter(
             ...
             width = .25,
             alpha = .5) +
  geom_smooth(data = rq2_fatigue_post_graph_m,
              aes(y = response, 
                  ymin = response - SE,
                  ymax = response + SE,
                  x = lextale_raw, 
...
ggsave(here::here("figures_study3", "rq2_lf_scatterplot.jpeg"),
       dpi = 300,
       width = 7.5,
       height = 4,
       unit = "in")
...
rq2_h_adj <- rbind(
  ...
  mutate(outcome = factor(outcome, levels = c("intelligibility", "effort",
                                              "fatigue"))) |> 
  mutate(p.adj = p.adjust(p.orig, method = "BH"))
  

rq2_h_adj <- merge(rq2_h_adj, 
      BHAdjust(rq2_h_adj$p.orig, FDR = 0.05, arr = "p.orig"),
      all = T)  |> 
...
rq2_h_adj |> 
  select(outcome, variable, lang_status, stimulus_type,
         Chisq, Df, pvalue_text, fdr_pvalue_text, significant_after_FDR) |> 
...
rq2_h_adj_spline <- merge(rq2_h_adj_spline, 
      BHAdjust(rq2_h_adj_spline$p.orig, FDR = 0.05, arr = "p.orig"),
      all = T)  |> 
  arrange(stimulus_type) |> 
  arrange(lang_status) |> 
  arrange(variable) |> 
...
### ---- 2-way interaction; 2 dataset
##### model: word recognition score/ intelligibility  
###### checking value add of using dispersion, which variable
> Using separate datasets

```{r}
rq2_intell1_b  <- glmmTMB::glmmTMB(intell_t ~ kbit2_raw_c + block_number +
                                    lextale_raw_c + stimulus_type_hp +
                                    lextale_raw_c:stimulus_type_hp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
                      dispformula = ~ stimulus_type_hp,
                      data = data_boothroyd_b,
                      family = glmmTMB::beta_family()) 

rq2_intell2_b  <- glmmTMB::glmmTMB(intell_t ~ kbit2_raw_c + block_number +
                                    lextale_raw_c + stimulus_type_hp +
                                    lextale_raw_c:stimulus_type_hp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
...
rq2_intell3_b  <- glmmTMB::glmmTMB(intell_t ~ kbit2_raw_c + block_number +
                                    lextale_raw_c + stimulus_type_hp +
                                    lextale_raw_c:stimulus_type_hp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
...
AIC(rq2_intell1_b, rq2_intell2_b, rq2_intell3_b) |> 
  arrange(AIC)

bbmle::AICtab(rq2_intell1_b, rq2_intell2_b, rq2_intell3_b)
```

```{r}
rq2_intell1_m  <- glmmTMB::glmmTMB(intell_t ~ kbit2_raw_c + block_number +
                                    lextale_raw_c + stimulus_type_hp +
                                    lextale_raw_c:stimulus_type_hp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
                      dispformula = ~ stimulus_type_hp,
                      data = data_boothroyd_m,
                      family = glmmTMB::beta_family())

rq2_intell2_m  <- glmmTMB::glmmTMB(intell_t ~ kbit2_raw_c + block_number +
                                    lextale_raw_c + stimulus_type_hp +
                                    lextale_raw_c:stimulus_type_hp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
...
rq2_intell3_m  <- glmmTMB::glmmTMB(intell_t ~ kbit2_raw_c + block_number +
                                    lextale_raw_c + stimulus_type_hp +
                                    lextale_raw_c:stimulus_type_hp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
...
AIC(rq2_intell1_m, rq2_intell2_m, rq2_intell3_m) |> 
  arrange(AIC)

bbmle::AICtab(rq2_intell1_m, rq2_intell2_m, rq2_intell3_m)
...
```{r}
#bilingual
rq2_intell3_noSpline_b  <- glmmTMB::glmmTMB(intell_t ~ kbit2_raw_c + block_number +
                                    lextale_raw_c + stimulus_type_hp +
                                    lextale_raw_c:stimulus_type_hp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
...
rq2_intell3_Spline_2df_b  <- glmmTMB::glmmTMB(intell_t ~ kbit2_raw_c + block_number +
                                    splines::ns(lextale_raw_c, df = 2) 
                                    + stimulus_type_hp +
                                    splines::ns(lextale_raw_c, df = 2):stimulus_type_hp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
...
rq2_intell3_Spline_3df_b  <- glmmTMB::glmmTMB(intell_t ~ kbit2_raw_c + block_number +
                                    splines::ns(lextale_raw_c, df = 3) 
                                    + stimulus_type_hp +
                                    splines::ns(lextale_raw_c, df = 3):stimulus_type_hp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
...
rq2_intell3_Spline_4df_b  <- glmmTMB::glmmTMB(intell_t ~ kbit2_raw_c + block_number +
                                    splines::ns(lextale_raw_c, df = 4) 
                                    + stimulus_type_hp +
                                    splines::ns(lextale_raw_c, df = 4):stimulus_type_hp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
...
lmtest::lrtest(rq2_intell3_noSpline_b,
               rq2_intell3_Spline_2df_b)

lmtest::lrtest(rq2_intell3_noSpline_b,
               rq2_intell3_Spline_3df_b)

lmtest::lrtest(rq2_intell3_noSpline_b,
               rq2_intell3_Spline_4df_b)

lmtest::lrtest(rq2_intell3_Spline_2df_b,
               rq2_intell3_Spline_3df_b)

lmtest::lrtest(rq2_intell3_Spline_2df_b,
               rq2_intell3_Spline_4df_b)

AIC(rq2_intell3_noSpline_b, rq2_intell3_Spline_2df_b,
    rq2_intell3_Spline_3df_b, rq2_intell3_Spline_4df_b) |>
  arrange(AIC)

bbmle::AICtab(rq2_intell3_noSpline_b, rq2_intell3_Spline_2df_b,
    rq2_intell3_Spline_3df_b, rq2_intell3_Spline_4df_b)
```
...
```{r}
#monolingual
rq2_intell3_noSpline_m  <- glmmTMB::glmmTMB(intell_t ~ kbit2_raw_c + block_number +
                                    lextale_raw_c + stimulus_type_hp +
                                    lextale_raw_c:stimulus_type_hp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
...
rq2_intell3_Spline_2df_m  <- glmmTMB::glmmTMB(intell_t ~ kbit2_raw_c + block_number +
                                    splines::ns(lextale_raw_c, df = 2) 
                                    + stimulus_type_hp +
                                    splines::ns(lextale_raw_c, df = 2):stimulus_type_hp +
                                    (1 | listener_id) + 
...
rq2_intell3_Spline_3df_m  <- glmmTMB::glmmTMB(intell_t ~ kbit2_raw_c + block_number +
                                    splines::ns(lextale_raw_c, df = 3) 
                                    + stimulus_type_hp +
                                    splines::ns(lextale_raw_c, df = 3):stimulus_type_hp +
                                    (1 | listener_id) + 
...
rq2_intell3_Spline_4df_m  <- glmmTMB::glmmTMB(intell_t ~ kbit2_raw_c + block_number +
                                    splines::ns(lextale_raw_c, df = 4) 
                                    + stimulus_type_hp +
                                    splines::ns(lextale_raw_c, df = 4):stimulus_type_hp +
                                    (1 | listener_id) + 
                      ...
                      data = data_boothroyd_m,
                      family = glmmTMB::beta_family())
#4df has a convergence problem

lmtest::lrtest(rq2_intell3_noSpline_m,
               rq2_intell3_Spline_2df_m)

lmtest::lrtest(rq2_intell3_noSpline_m,
               rq2_intell3_Spline_3df_m)

lmtest::lrtest(rq2_intell3_noSpline_m,
               rq2_intell3_Spline_4df_m)

lmtest::lrtest(rq2_intell3_Spline_2df_m,
               rq2_intell3_Spline_3df_m)

lmtest::lrtest(rq2_intell3_Spline_2df_m,
               rq2_intell3_Spline_4df_m)

AIC(rq2_intell3_noSpline_m, rq2_intell3_Spline_2df_m,
    rq2_intell3_Spline_3df_m, rq2_intell3_Spline_4df_m) |>
  arrange(AIC)

bbmle::AICtab(rq2_intell3_noSpline_m, rq2_intell3_Spline_2df_m,
    rq2_intell3_Spline_3df_m, rq2_intell3_Spline_4df_m)
```
...
view(rq2_intell2_h_b) 

```

```{r}
rq2_intell2_h_m <- rbind(
car::Anova(rq2_intell2_hp_m, type = "III") |> 
data.frame() |>  
    rename(p.orig = Pr..Chisq.) |> 
...
view(rq2_intell2_h_m) 
```

###### table HP-LP difference
```{r}
#bilingual
rg_rq2_intell2_b <- emmeans::ref_grid(rq2_intell2_hp_b, at = list(lextale_raw_c = 
                                      c(60,70,80,90,100)))
                            

rq2_intell2_pred_b <- emmeans::emmeans(rg_rq2_intell2_b, 
                                       ~ stimulus_type_hp * lextale_raw_c,
                                       type = "response") |> 
  as.data.frame() |> 
...
view(rq2_intell2_pred_b)
```

```{r}
#monolingual
rg_rq2_intell2_m <- emmeans::ref_grid(rq2_intell2_hp_m, at = list(lextale_raw_c = 
                                      c(80,90,100)))
                            

rq2_intell2_pred_m <- emmeans::emmeans(rg_rq2_intell2_m, 
                                       ~ stimulus_type_hp * lextale_raw_c,
                                       type = "response") |> 
  as.data.frame() |> 
  mutate(bilingual ="Monolingual",
         response = round((response - .0125) / (.975),2),
         SE = round((SE- .0125) / (.975),2),
         asymp.LCL = round((asymp.LCL- .0125) / (.975),2),
         asymp.UCL = round((asymp.UCL- .0125) / (.975),2)) |> 
...
view(rq2_intell2_pred_m)
```

```{r}
rq2_intell2_pred <- merge(rq2_intell2_pred_b, rq2_intell2_pred_m, all = T) |> 
  mutate(`Language status` = factor(`Language status`, levels = c("Bilingual",
                                                                  "Monolingual"))) |> 
  arrange(`Language status`)
...
###### table lextale difference 
```{r}
#bilingual
rg_rq2_intell2_b <- emmeans::ref_grid(rq2_intell2_hp_b, at = list(lextale_raw_c = 
                                      c(60,70,80,90,100)))

rq2_intell2_pred_b <- emmeans::emmeans(rg_rq2_intell2_b, ~ stimulus_type_hp * lextale_raw_c,
                                      type = "response") |> 
  as.data.frame() |> 
  mutate(lang_status = "Bilingual",
         response = round((response - .0125) / (.975),2),
         SE = round((SE- .0125) / (.975),2),
         asymp.LCL = round((asymp.LCL- .0125) / (.975),2),
         asymp.UCL = round((asymp.UCL- .0125) / (.975),2)) |> 
  ...
  pivot_wider(names_from = c(lextale_raw_c),
              values_from = c(response, SE, asymp.LCL, asymp.UCL)) |> 
  mutate(`difference 60-70` = response_70 - response_60,
         `difference 70-80` = response_80 - response_70,
         `difference 80-90` = response_90 - response_80,
         `difference 90-100` = response_100 - response_90,) |>
  rename(`Language status` = lang_status) |> 
  rename(`Utterance type` = stimulus_type_hp) |> 
  rename(`Intell based on LexTALE score as 60` = response_60) |> 
  rename(`Intell based on LexTALE score as 70` = response_70) |> 
  rename(`Intell based on LexTALE score as 80` = response_80) |> 
  rename(`Intell based on LexTALE score as 90` = response_90) |> 
  rename(`Intell based on LexTALE score as 100` = response_100) |> 
  select(`Utterance type`, 
         `Language status`, 
         `Intell based on LexTALE score as 60`,
         `Intell based on LexTALE score as 70`,
         `Intell based on LexTALE score as 80`,
         `Intell based on LexTALE score as 90`,
         `Intell based on LexTALE score as 100`,
         `difference 60-70`,
         `difference 70-80`,
         `difference 80-90`,
         `difference 90-100`)
...
rq2_intell2_pred_m <- emmeans::emmeans(rg_rq2_intell2_m, ~ stimulus_type_hp * lextale_raw_c,
                                      type = "response") |> 
  as.data.frame() |> 
  mutate(lang_status = "Monolingual",
         response = round((response - .0125) / (.975),2),
         SE = round((SE- .0125) / (.975),2),
         asymp.LCL = round((asymp.LCL- .0125) / (.975),2),
         asymp.UCL = round((asymp.UCL- .0125) / (.975),2)) |> 
  ...
  pivot_wider(names_from = c(lextale_raw_c),
              values_from = c(response, SE, asymp.LCL, asymp.UCL)) |> 
  mutate(`difference 80-90` = response_90 - response_80,
         `difference 90-100` = response_100 - response_90,) |>
  rename(`Language status` = lang_status) |> 
  rename(`Utterance type` = stimulus_type_hp) |> 
  rename(`Intell based on LexTALE score as 80` = response_80) |> 
...
rq2_intell2_pred <- merge(rq2_intell2_pred_b, rq2_intell2_pred_m, all = T) |> 
  mutate(`Language status` = factor(`Language status`, levels = c("Bilingual",
                                                                  "Monolingual"))) |> 
  arrange(`Language status`) |> 
  relocate(`Intell based on LexTALE score as 60`, 
           .before = `Intell based on LexTALE score as 80`) |> 
  relocate(`Intell based on LexTALE score as 70`, 
           .before = `Intell based on LexTALE score as 80`) |> 
  relocate(`difference 60-70`, 
           .before = `difference 80-90`) |> 
  relocate(`difference 70-80`, 
           .before = `difference 80-90`)

rq2_intell2_pred[is.na(rq2_intell2_pred)] <- "-"
...
rq2_intell2_graph_b <- emmeans::emmeans(rg_rq2_intell2_b, 
                                        ~ stimulus_type_hp * lextale_raw_c,
                                      type = "response") |> 
  as.data.frame() |> 
  mutate(response = (response - .0125) / (.975),
         SE = (SE- .0125) / (.975),
         asymp.LCL = (asymp.LCL- .0125) / (.975),
         asymp.UCL = (asymp.UCL- .0125) / (.975),
         lang_status = "Bilingual") 
...
rq2_intell2_graph_m <- emmeans::emmeans(rg_rq2_intell2_m, 
                                       ~ stimulus_type_hp * lextale_raw_c,
                                      type = "response") |> 
  as.data.frame() |> 
  mutate(response = (response - .0125) / (.975),
...
view(rq2_intell2_graph_m)


```

```{r}
sig_text <- data.frame(lextale_raw_c = c(62, 62, 62),
                       response = c(.99, .85, .70),
                       bilingual = c("Bilingual","Bilingual","Bilingual"),
                       lab = c("*","*","*"))

p1 <- data_boothroyd_b |> 
  mutate(lang_status = case_match(lang_status,
  ...
  geom_point(aes(y = intelligibility, 
                 ...
                 colour = stimulus_type),
             alpha = .5) +
  geom_smooth(data = rq2_intell2_graph_b,
              aes(y = response, 
                  ymin = response - SE,
                  ymax = response + SE,
                  x = lextale_raw_c, 
  ...
  geom_text(data = sig_text, 
            aes(y = response, 
                x = lextale_raw_c),
            label = c("*","*","*")) +
  geom_segment(aes(x = 59, xend = 59, y = .67, yend = .96), size = 0.2) +
  geom_segment(aes(x = 59, xend = 59.5, y = .67, yend = .67), size = 0.2) +
  geom_segment(aes(x = 59, xend = 59.5, y = .82, yend = .82), size = 0.2) +
  geom_segment(aes(x = 59, xend = 59.5, y = .96, yend = .96), size = 0.2) +
  annotate("text", x = 57.5, y = .82, label = "A*",
...
p2 <- data_boothroyd_m |> 
  ...
  geom_point(aes(y = intelligibility, 
                 ...
                 colour = stimulus_type),
             alpha = .5) +
  geom_smooth(data = rq2_intell2_graph_m,
              aes(y = response, 
                  ymin = response - SE,
                  ymax = response + SE,
                  x = lextale_raw_c, 
...
```{r}
a3rq2_wr2_fig <- ggpubr::ggarrange(p1, p2,
                  nrow = 1,
                  ncol = 2, 
                  widths = c(1.25,1),
                  common.legend = TRUE) 

ggpubr::annotate_figure(a3rq2_wr2_fig,
                        bottom = ggpubr::text_grob(
                          "Proficiency on vocabolary knowledge (LexTALE scores)", 
                        family = "Times",
                        vjust = -.1)) 
...
```{r}
rq2_effort1_b  <- glmmTMB::glmmTMB(effort_t ~ kbit2_raw_c + block_number +
                                    lextale_raw_c + stimulus_type_hp +
                                    lextale_raw_c:stimulus_type_hp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
...
rq2_effort2_b  <- glmmTMB::glmmTMB(effort_t ~ kbit2_raw_c + block_number +
                                    lextale_raw_c + stimulus_type_hp +
                                    lextale_raw_c:stimulus_type_hp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
...
rq2_effort3_b  <- glmmTMB::glmmTMB(effort_t ~ kbit2_raw_c + block_number +
                                    lextale_raw_c + stimulus_type_hp +
                                    lextale_raw_c:stimulus_type_hp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
...
AIC(rq2_effort1_b, rq2_effort2_b, rq2_effort3_b) |> 
  arrange(AIC)

bbmle::AICtab(rq2_effort1_b, rq2_effort2_b, rq2_effort3_b)
```

```{r}
rq2_effort1_m  <- glmmTMB::glmmTMB(effort_t ~ kbit2_raw_c + block_number +
                                    lextale_raw_c + stimulus_type_hp +
                                    lextale_raw_c:stimulus_type_hp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
...
rq2_effort2_m  <- glmmTMB::glmmTMB(effort_t ~ kbit2_raw_c + block_number +
                                    lextale_raw_c + stimulus_type_hp +
                                    lextale_raw_c:stimulus_type_hp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
...
rq2_effort3_m  <- glmmTMB::glmmTMB(effort_t ~ kbit2_raw_c + block_number +
                                    lextale_raw_c + stimulus_type_hp +
                                    lextale_raw_c:stimulus_type_hp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
...
AIC(rq2_effort1_m, rq2_effort2_m, rq2_effort3_m) |> 
  arrange(AIC)

bbmle::AICtab(rq2_effort1_m, rq2_effort2_m, rq2_effort3_m)
...
```{r}
#bilingual
rq2_effort3_noSpline_b  <- glmmTMB::glmmTMB(effort_t ~ kbit2_raw_c + block_number +
                                    lextale_raw_c + stimulus_type_hp +
                                    lextale_raw_c:stimulus_type_hp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
...
rq2_effort3_Spline_2df_b  <- glmmTMB::glmmTMB(effort_t ~ kbit2_raw_c + block_number +
                                    splines::ns(lextale_raw_c, df = 2) 
                                    + stimulus_type_hp +
                                    splines::ns(lextale_raw_c, df = 2):stimulus_type_hp +
                                    (1 | listener_id) + 
...
rq2_effort3_Spline_3df_b  <- glmmTMB::glmmTMB(effort_t ~ kbit2_raw_c + block_number +
                                    splines::ns(lextale_raw_c, df = 3) 
                                    + stimulus_type_hp +
                                    splines::ns(lextale_raw_c, df = 3):stimulus_type_hp +
                                    (1 | listener_id) + 
...
rq2_effort3_Spline_4df_b  <- glmmTMB::glmmTMB(effort_t ~ kbit2_raw_c + block_number +
                                    splines::ns(lextale_raw_c, df = 4) 
                                    + stimulus_type_hp +
                                    splines::ns(lextale_raw_c, df = 4):stimulus_type_hp +
                                    (1 | listener_id) + 
...
lmtest::lrtest(rq2_effort3_noSpline_b,
               rq2_effort3_Spline_2df_b)

lmtest::lrtest(rq2_effort3_noSpline_b,
               rq2_effort3_Spline_3df_b)

lmtest::lrtest(rq2_effort3_noSpline_b,
               rq2_effort3_Spline_4df_b)

lmtest::lrtest(rq2_effort3_Spline_2df_b,
               rq2_effort3_Spline_3df_b)

lmtest::lrtest(rq2_effort3_Spline_2df_b,
               rq2_effort3_Spline_4df_b)

AIC(rq2_effort3_noSpline_b, rq2_effort3_Spline_2df_b,
    rq2_effort3_Spline_3df_b, rq2_effort3_Spline_4df_b) |>
  arrange(AIC)

bbmle::AICtab(rq2_effort3_noSpline_b, rq2_effort3_Spline_2df_b,
    rq2_effort3_Spline_3df_b, rq2_effort3_Spline_4df_b)
```
...
```{r}
#monolingual
rq2_effort3_noSpline_m  <- glmmTMB::glmmTMB(effort_t ~ kbit2_raw_c + block_number +
                                    lextale_raw_c + stimulus_type_hp +
                                    lextale_raw_c:stimulus_type_hp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
...
rq2_effort3_Spline_2df_m  <- glmmTMB::glmmTMB(effort_t ~ kbit2_raw_c + block_number +
                                    splines::ns(lextale_raw_c, df = 2) 
                                    + stimulus_type_hp +
                                    splines::ns(lextale_raw_c, df = 2):stimulus_type_hp +
                                    (1 | listener_id) + 
...
rq2_effort3_Spline_3df_m  <- glmmTMB::glmmTMB(effort_t ~ kbit2_raw_c + block_number +
                                    splines::ns(lextale_raw_c, df = 3) 
                                    + stimulus_type_hp +
                                    splines::ns(lextale_raw_c, df = 3):stimulus_type_hp +
                                    (1 | listener_id) + 
...
rq2_effort3_Spline_4df_m  <- glmmTMB::glmmTMB(effort_t ~ kbit2_raw_c + block_number +
                                    splines::ns(lextale_raw_c, df = 4) 
                                    + stimulus_type_hp +
                                    splines::ns(lextale_raw_c, df = 4):stimulus_type_hp +
                                    (1 | listener_id) + 
...
lmtest::lrtest(rq2_effort3_noSpline_m,
               rq2_effort3_Spline_2df_m)

lmtest::lrtest(rq2_effort3_noSpline_m,
               rq2_effort3_Spline_3df_m)

lmtest::lrtest(rq2_effort3_noSpline_m,
               rq2_effort3_Spline_4df_m)

lmtest::lrtest(rq2_effort3_Spline_2df_m,
               rq2_effort3_Spline_3df_m)

lmtest::lrtest(rq2_effort3_Spline_2df_m,
               rq2_effort3_Spline_4df_m)

AIC(rq2_effort3_noSpline_m, rq2_effort3_Spline_2df_m,
    rq2_effort3_Spline_3df_m, rq2_effort3_Spline_4df_m) |>
  arrange(AIC)

bbmle::AICtab(rq2_effort3_noSpline_m, rq2_effort3_Spline_2df_m,
    rq2_effort3_Spline_3df_m, rq2_effort3_Spline_4df_m)
```
...
```{r}
#HP utterances as reference
rq2_effort2_hp_b <- glmmTMB::glmmTMB(effort_t ~ kbit2_raw_c + block_number +
                                    lextale_raw_c + stimulus_type_hp +
                                    lextale_raw_c:stimulus_type_hp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
...
summary(rq2_effort2_hp_b)

```

```{r}
#LP utterances as reference
rq2_effort2_lp_b <- glmmTMB::glmmTMB(effort_t ~ kbit2_raw_c + block_number +
                                    lextale_raw_c + stimulus_type_lp +
                                    lextale_raw_c:stimulus_type_lp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
...
summary(rq2_effort2_lp_b)
```

```{r}
#ZP utterances as reference
rq2_effort2_zp_b <- glmmTMB::glmmTMB(effort_t ~ kbit2_raw_c + block_number +
                                    lextale_raw_c + stimulus_type_zp +
                                    lextale_raw_c:stimulus_type_zp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
...
summary(rq2_effort2_zp_b)
```


```{r}
#HP utterances as reference
rq2_effort2_hp_m <- glmmTMB::glmmTMB(effort_t ~ kbit2_raw_c + block_number +
                                    lextale_raw_c + stimulus_type_hp +
                                    lextale_raw_c:stimulus_type_hp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
...
summary(rq2_effort2_hp_m)

```

```{r}
#LP utterances as reference
rq2_effort2_lp_m <- glmmTMB::glmmTMB(effort_t ~ kbit2_raw_c + block_number +
                                    lextale_raw_c + stimulus_type_lp +
                                    lextale_raw_c:stimulus_type_lp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
...
summary(rq2_effort2_lp_m)
```

```{r}
#ZP utterances as reference
#bilingual = bilingual 0; monolingual 1; other coef pertain to bilinguals
rq2_effort2_zp_m <- glmmTMB::glmmTMB(effort_t ~ kbit2_raw_c + block_number +
                                    lextale_raw_c + stimulus_type_zp +
                                    lextale_raw_c:stimulus_type_zp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
...
```{r}
car::Anova(rq2_effort2_hp_b, type = "III")
car::Anova(rq2_effort2_lp_b, type = "III")
car::Anova(rq2_effort2_zp_b, type = "III")
car::Anova(rq2_effort2_hp_m, type = "III")
car::Anova(rq2_effort2_lp_m, type = "III")
car::Anova(rq2_effort2_zp_m, type = "III")
...
```{r}
rq2_effort2_h_b <- rbind(
car::Anova(rq2_effort2_hp_b, type = "III") |> 
data.frame() |>  
    rename(p.orig = Pr..Chisq.) |> 
    mutate(variable = rownames(car::Anova(rq2_effort2_hp_b, type = "III")),
           lang_status = "Bilingual",
           stimulus_type = "HP") |> 
    relocate(variable, .before = Chisq) |> 
    relocate(lang_status, .before = Chisq) |> 
...
```{r}
rq2_effort2_h_m <- rbind(
car::Anova(rq2_effort2_hp_m, type = "III") |> 
data.frame() |>  
    rename(p.orig = Pr..Chisq.) |> 
    mutate(variable = rownames(car::Anova(rq2_effort2_hp_m, type = "III")),
           lang_status = "Monolingual",
           stimulus_type = "HP") |> 
    relocate(variable, .before = Chisq) |> 
    relocate(lang_status, .before = Chisq) |> 
...
```{r}
rq2_fatigue_post1_b  <- glmmTMB::glmmTMB(fatigue_post_t ~ kbit2_raw_c + block_number +
                                    lextale_raw_c + stimulus_type_hp +
                                    lextale_raw_c:stimulus_type_hp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
...
rq2_fatigue_post2_b  <- glmmTMB::glmmTMB(fatigue_post_t ~ kbit2_raw_c + block_number +
                                    lextale_raw_c + stimulus_type_hp +
                                    lextale_raw_c:stimulus_type_hp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
...
rq2_fatigue_post3_b  <- glmmTMB::glmmTMB(fatigue_post_t ~ kbit2_raw_c + block_number +
                                    lextale_raw_c + stimulus_type_hp +
                                    lextale_raw_c:stimulus_type_hp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
...
AIC(rq2_fatigue_post1_b, rq2_fatigue_post2_b, rq2_fatigue_post3_b) |> 
  arrange(AIC)

bbmle::AICtab(rq2_fatigue_post1_b, rq2_fatigue_post2_b, rq2_fatigue_post3_b)
```

```{r}
rq2_fatigue_post1_m  <- glmmTMB::glmmTMB(fatigue_post_t ~ kbit2_raw_c + block_number +
                                    lextale_raw_c + stimulus_type_hp +
                                    lextale_raw_c:stimulus_type_hp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
...
rq2_fatigue_post2_m  <- glmmTMB::glmmTMB(fatigue_post_t ~ kbit2_raw_c + block_number +
                                    lextale_raw_c + stimulus_type_hp +
                                    lextale_raw_c:stimulus_type_hp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
...
rq2_fatigue_post3_m <- glmmTMB::glmmTMB(fatigue_post_t ~ kbit2_raw_c + block_number +
                                    lextale_raw_c + stimulus_type_hp +
                                    lextale_raw_c:stimulus_type_hp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
...
AIC(rq2_fatigue_post1_m, rq2_fatigue_post2_m, rq2_fatigue_post3_m) |> 
  arrange(AIC)

bbmle::AICtab(rq2_fatigue_post1_m, rq2_fatigue_post2_m, rq2_fatigue_post3_m)
...
```{r}
#bilingual
rq2_fatigue_post3_noSpline_b  <- glmmTMB::glmmTMB(fatigue_post_t ~ kbit2_raw_c + block_number +
                                    lextale_raw_c + stimulus_type_hp +
                                    lextale_raw_c:stimulus_type_hp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
...
rq2_fatigue_post3_Spline_2df_b  <- glmmTMB::glmmTMB(fatigue_post_t ~ kbit2_raw_c + block_number +
                                    splines::ns(lextale_raw_c, df = 2) 
                                    + stimulus_type_hp +
                                    splines::ns(lextale_raw_c, df = 2):stimulus_type_hp +
                                    (1 | listener_id) + 
...
rq2_fatigue_post3_Spline_3df_b  <- glmmTMB::glmmTMB(fatigue_post_t ~ kbit2_raw_c + block_number +
                                    splines::ns(lextale_raw_c, df = 3) 
                                    + stimulus_type_hp +
                                    splines::ns(lextale_raw_c, df = 3):stimulus_type_hp +
                                    (1 | listener_id) + 
...
rq2_fatigue_post3_Spline_4df_b  <- glmmTMB::glmmTMB(fatigue_post_t ~ kbit2_raw_c + block_number +
                                    splines::ns(lextale_raw_c, df = 4) 
                                    + stimulus_type_hp +
                                    splines::ns(lextale_raw_c, df = 4):stimulus_type_hp +
                                    (1 | listener_id) + 
...
lmtest::lrtest(rq2_fatigue_post3_noSpline_b,
               rq2_fatigue_post3_Spline_2df_b)

lmtest::lrtest(rq2_fatigue_post3_noSpline_b,
               rq2_fatigue_post3_Spline_3df_b)

lmtest::lrtest(rq2_fatigue_post3_noSpline_b,
               rq2_fatigue_post3_Spline_4df_b)

lmtest::lrtest(rq2_fatigue_post3_Spline_2df_b,
               rq2_fatigue_post3_Spline_3df_b)

lmtest::lrtest(rq2_fatigue_post3_Spline_2df_b,
               rq2_fatigue_post3_Spline_4df_b)

AIC(rq2_fatigue_post3_noSpline_b, rq2_fatigue_post3_Spline_2df_b,
    rq2_fatigue_post3_Spline_3df_b, rq2_fatigue_post3_Spline_4df_b) |>
  arrange(AIC)

bbmle::AICtab(rq2_fatigue_post3_noSpline_b, rq2_fatigue_post3_Spline_2df_b,
    rq2_fatigue_post3_Spline_3df_b, rq2_fatigue_post3_Spline_4df_b)
```
...
```{r}
#monolingual
rq2_fatigue_post3_noSpline_m  <- glmmTMB::glmmTMB(fatigue_post_t ~ kbit2_raw_c + block_number +
                                    lextale_raw_c + stimulus_type_hp +
                                    lextale_raw_c:stimulus_type_hp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
...
rq2_fatigue_post3_Spline_2df_m  <- glmmTMB::glmmTMB(fatigue_post_t ~ kbit2_raw_c + block_number +
                                    splines::ns(lextale_raw_c, df = 2) 
                                    + stimulus_type_hp +
                                    splines::ns(lextale_raw_c, df = 2):stimulus_type_hp +
                                    (1 | listener_id) + 
...
rq2_fatigue_post3_Spline_3df_m  <- glmmTMB::glmmTMB(fatigue_post_t ~ kbit2_raw_c + block_number +
                                    splines::ns(lextale_raw_c, df = 3) 
                                    + stimulus_type_hp +
                                    splines::ns(lextale_raw_c, df = 3):stimulus_type_hp +
                                    (1 | listener_id) + 
...
rq2_fatigue_post3_Spline_4df_m  <- glmmTMB::glmmTMB(fatigue_post_t ~ kbit2_raw_c + block_number +
                                    splines::ns(lextale_raw_c, df = 4) 
                                    + stimulus_type_hp +
                                    splines::ns(lextale_raw_c, df = 4):stimulus_type_hp +
                                    (1 | listener_id) + 
...
lmtest::lrtest(rq2_fatigue_post3_noSpline_m,
               rq2_fatigue_post3_Spline_2df_m)

lmtest::lrtest(rq2_fatigue_post3_noSpline_m,
               rq2_fatigue_post3_Spline_3df_m)

lmtest::lrtest(rq2_fatigue_post3_noSpline_m,
               rq2_fatigue_post3_Spline_4df_m)

lmtest::lrtest(rq2_fatigue_post3_Spline_2df_m,
               rq2_fatigue_post3_Spline_3df_m)

lmtest::lrtest(rq2_fatigue_post3_Spline_2df_m,
               rq2_fatigue_post3_Spline_4df_m)

AIC(rq2_fatigue_post3_noSpline_m, rq2_fatigue_post3_Spline_2df_m,
    rq2_fatigue_post3_Spline_3df_m, rq2_fatigue_post3_Spline_4df_m) |>
  arrange(AIC)

bbmle::AICtab(rq2_fatigue_post3_noSpline_m, rq2_fatigue_post3_Spline_2df_m,
    rq2_fatigue_post3_Spline_3df_m, rq2_fatigue_post3_Spline_4df_m)
```
...
```{r}
#HP utterances as reference
rq2_fatigue_post2_hp_b <- glmmTMB::glmmTMB(fatigue_post_t ~ kbit2_raw_c + block_number +
                                    lextale_raw_c + stimulus_type_hp +
                                    lextale_raw_c:stimulus_type_hp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
...
summary(rq2_fatigue_post2_hp_b)

```

```{r}
#LP utterances as reference
rq2_fatigue_post2_lp_b <- glmmTMB::glmmTMB(fatigue_post_t ~ kbit2_raw_c + block_number +
                                    lextale_raw_c + stimulus_type_lp +
                                    lextale_raw_c:stimulus_type_lp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
...
summary(rq2_fatigue_post2_lp_b)
```

```{r}
#ZP utterances as reference
rq2_fatigue_post2_zp_b <- glmmTMB::glmmTMB(fatigue_post_t ~ kbit2_raw_c + block_number +
                                    lextale_raw_c + stimulus_type_zp +
                                    lextale_raw_c:stimulus_type_zp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
...
summary(rq2_fatigue_post2_zp_b)
```


```{r}
#HP utterances as reference
rq2_fatigue_post2_hp_m <- glmmTMB::glmmTMB(fatigue_post_t ~ kbit2_raw_c + block_number +
                                    lextale_raw_c + stimulus_type_hp +
                                    lextale_raw_c:stimulus_type_hp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
...
summary(rq2_fatigue_post2_hp_m)

```

```{r}
#LP utterances as reference
rq2_fatigue_post2_lp_m <- glmmTMB::glmmTMB(fatigue_post_t ~ kbit2_raw_c + block_number +
                                    lextale_raw_c + stimulus_type_lp +
                                    lextale_raw_c:stimulus_type_lp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
...
summary(rq2_fatigue_post2_lp_m)
```

```{r}
#ZP utterances as reference
#bilingual = bilingual 0; monolingual 1; other coef pertain to bilinguals
rq2_fatigue_post2_zp_m <- glmmTMB::glmmTMB(fatigue_post_t ~ kbit2_raw_c + block_number +
                                    lextale_raw_c + stimulus_type_zp +
                                    lextale_raw_c:stimulus_type_zp +
                                    (1 | listener_id) + 
                                    (1 | speaker_id),
...
summary(rq2_fatigue_post2_zp_m)
```
...
```{r}
car::Anova(rq2_fatigue_post2_hp_b, type = "III")
car::Anova(rq2_fatigue_post2_lp_b, type = "III")
car::Anova(rq2_fatigue_post2_zp_b, type = "III")
car::Anova(rq2_fatigue_post2_hp_m, type = "III")
car::Anova(rq2_fatigue_post2_lp_m, type = "III")
car::Anova(rq2_fatigue_post2_zp_m, type = "III")
...
```{r}
rq2_fatigue_post2_h_b <- rbind(
car::Anova(rq2_fatigue_post2_hp_b, type = "III") |> 
data.frame() |>  
    rename(p.orig = Pr..Chisq.) |> 
    mutate(variable = rownames(car::Anova(rq2_fatigue_post2_hp_b, type = "III")),
           lang_status = "Bilingual",
           stimulus_type = "HP") |> 
    relocate(variable, .before = Chisq) |> 
    relocate(lang_status, .before = Chisq) |> 
...
view(rq2_fatigue_post2_h_b) 
```

```{r}
rq2_fatigue_post2_h_m <- rbind(
car::Anova(rq2_fatigue_post2_hp_m, type = "III") |> 
data.frame() |>  
    rename(p.orig = Pr..Chisq.) |> 
    mutate(variable = rownames(car::Anova(rq2_fatigue_post2_hp_m, type = "III")),
           lang_status = "Monolingual",
           stimulus_type = "HP") |> 
    relocate(variable, .before = Chisq) |> 
    relocate(lang_status, .before = Chisq) |> 
...
rq2_h_adj2_b <- merge(rq2_h_adj2_b, 
      BHAdjust(rq2_h_adj2_b$p.orig, FDR = 0.05, arr = "p.orig"),
      all = T)  |> 
  arrange(stimulus_type) |> 
  arrange(lang_status) |> 
  arrange(variable) |> 
...
```{r}
#monolingual
p <- rq2_h_adj2_m$p.orig
u <- sort(p)
uThresh <- .05 * (1:length(p))/length(p)
k <- max(c(which((u<=uThresh)), 0), na.rm = TRUE)
pCrit <- u[k]
list(BHSig = c(rep(TRUE, k), rep(FALSE, length(p)-k))[order(order(p))]
                , pCrit = pCrit)
cbind(u,uThresh)
# no values in u were less or equal to UThresh; thus no threshold pvalue
# thus the code below is different; otherwise there will be an error
```
...
rq2_h_adj2_m |> 
  select(outcome, variable, lang_status, stimulus_type,
         Chisq, Df, pvalue_text, fdr_pvalue_text) |> 
  mutate(Chisq = round(Chisq,2)) |> 
  kableExtra::kbl() |> 
  kableExtra::kable_classic(html_font = "Times New Roman", full_width = F)
