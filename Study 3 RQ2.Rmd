...
rq2_intell_graph_b <- rq2_intell_pred |> 
  filter(bilingual == "Bilingual")

rq2_intell_graph_m <- rq2_intell_pred |> 
...
###### table HP-LP difference
```{r}
rg_rq2 <- emmeans::ref_grid(rq2_effort_hp_b, at = list(lextale_raw_c = 
                                      c(60,70,80,90,100)))
                            

rq2_effort_pred <- emmeans::emmeans(rg_rq2, ~ stimulus_type_hp * lextale_raw_c |
                                      bilingual,
                                      type = "response") |> 
  as.data.frame() |> 
  mutate(bilingual = case_match(bilingual,
...
rq2_effort_graph <- emmeans::emmeans(rg_rq2, ~ stimulus_type_hp * lextale_raw_c * 
                                      bilingual,
                                      type = "response") |> 
  as.data.frame() |> 
  mutate(bilingual = case_match(bilingual,
...
rg_rq2 <- emmeans::ref_grid(rq2_intell2_hp_b, at = list(lextale_raw_c = 
                                                         seq(60,100, by = 1.25)))
                            #rg.limit = 20000)

rq2_intell2_pred_b <- emmeans::emmeans(rg_rq2, ~ stimulus_type_hp * lextale_raw_c,
                                      type = "response") |> 
  as.data.frame() |> 
...
data_boothroyd_b |> 
  ...
  geom_point(
             ...
             alpha = .5) +
  geom_smooth(data = rq2_intell2_pred_b,
              aes(y = response, 
                  ymin = response - SE,
                  ymax = response + SE,
                  x = lextale_raw_c, 
...
```{r}
#getting marginal means given lextale scores between 0 to 100 by 1; average nonverbal 
rg_rq2 <- emmeans::ref_grid(rq2_intell2_hp_m, at = list(lextale_raw_c = 
                                                         seq(80,100, by = 1.25)))
                            #rg.limit = 20000)

rq2_intell2_pred_m <- emmeans::emmeans(rg_rq2, ~ stimulus_type_hp * lextale_raw_c,
                                      type = "response") |> 
  as.data.frame() |> 
  mutate(response = (response - .0125) / (.975),
         SE = (SE- .0125) / (.975),
...
data_boothroyd_m |> 
  ...
  geom_point(
             ...
             #position=position_jitterdodge(jitter.width=0.1),
             alpha = .5) +
  geom_smooth(data = rq2_intell2_pred_m,
              aes(y = response, 
                  ymin = response - SE,
                  ymax = response + SE,
                  x = lextale_raw_c, 
