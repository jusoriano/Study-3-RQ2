...
rq2_intell_pred <- emmeans::emmeans(rg_rq2_intell, ~ stimulus_type_hp * lextale_raw_c *
  ...
  mutate(bilingual = case_match(bilingual,
         ...
         asymp.UCL = round((asymp.UCL- .0125) / (.975),2),
         lextale_raw = mean(data_boothroyd$lextale_raw) + lextale_raw_c) |> 
  select(-c(df, lextale_raw_c)) |> 
  filter(lextale_raw == 60 |
           lextale_raw == 70 |
           lextale_raw == 80 |
           lextale_raw == 90 |
           lextale_raw == 100 ) |> 
  pivot_wider(names_from = c(lextale_raw),
...
rq2_effort_pred <- emmeans::emmeans(rg_rq2_effort, ~ stimulus_type_hp * lextale_raw_c *
  ...
  rename(`Language Status` = bilingual) |> 
  rename(`Utterance type` = stimulus_type_hp) |> 
  rename(`effort based on LexTALE score as 60` = response_60) |> 
  rename(`effort based on LexTALE score as 70` = response_70) |> 
  rename(`effort based on LexTALE score as 80` = response_80) |> 
  rename(`effort based on LexTALE score as 90` = response_90) |> 
  rename(`effort based on LexTALE score as 100` = response_100) |> 
  select(`Utterance type`, 
         `Language Status`, 
         `effort based on LexTALE score as 60`,
         `effort based on LexTALE score as 70`,
         `effort based on LexTALE score as 80`,
         `effort based on LexTALE score as 90`,
         `effort based on LexTALE score as 100`,
         `difference 60-70`,
         `difference 70-80`,
         `difference 80-90`,
         `difference 90-100`)
...
a3rq2_e_fig <- ggpubr::ggarrange(p1, p2,
                  nrow = 1,
                  ncol = 2, 
                  widths = c(1.25,1),
                  common.legend = TRUE) 

ggpubr::annotate_figure(a3rq2_e_fig,
                        bottom = ggpubr::text_grob(
                          "Proficiency on vocabolary knowledge (LexTALE scores)", 
                        family = "Times",
                        vjust = -.1)) 
...
rq2_fatigue_post_pred <- emmeans::emmeans(rg_rq2_fatigue_post, 
...
rq2_fatigue_post_pred <- emmeans::emmeans(rg_rq2_fatigue_post, 
  ...
  rename(`Language Status` = bilingual) |> 
  rename(`Utterance type` = stimulus_type_hp) |> 
  rename(`fatigue_post based on LexTALE score as 60` = response_60) |> 
  rename(`fatigue_post based on LexTALE score as 70` = response_70) |> 
  rename(`fatigue_post based on LexTALE score as 80` = response_80) |> 
  rename(`fatigue_post based on LexTALE score as 90` = response_90) |> 
  rename(`fatigue_post based on LexTALE score as 100` = response_100) |> 
  select(`Utterance type`, 
         `Language Status`, 
         `fatigue_post based on LexTALE score as 60`,
         `fatigue_post based on LexTALE score as 70`,
         `fatigue_post based on LexTALE score as 80`,
         `fatigue_post based on LexTALE score as 90`,
         `fatigue_post based on LexTALE score as 100`,
         `difference 60-70`,
         `difference 70-80`,
         `difference 80-90`,
         `difference 90-100`)
...
rq2_fatigue_post_graph <- emmeans::emmeans(rg_rq2_fatigue_post, 
                                           ~ stimulus_type_hp * lextale_raw_c * 
                                            bilingual,
                                            type = "response") |> 
  as.data.frame() |> 
